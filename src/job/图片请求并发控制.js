...
class ImageLoader {
  ...
  next() {
    ...
    if (this.index >= this.urls.length && this.currentCount === 0) {
      return this.resolve(this.results);
    }

    // 如果当前正在处理的请求数小于并发限制，并且有待处理的图片
    if (this.currentCount < this.concurrency && this.index < this.urls.length) {
      const url = this.urls[this.index++]; // 获取下一个图片URL，并增加索引
      this.currentCount++; // 增加当前处理的请求数

      // 加载图片
      this.loadImage(url)
        .then(result => {
          console.log("loadImage res", result);
          this.results.push(result); // 将成功加载的图片URL存储到results中
          this.currentCount--; // 减少当前处理的请求数
          this.next(); // 继续处理下一个图片
        })
        .catch(error => {
          console.error(`Error loading image: ${error}`); // 处理加载失败的情况
          this.currentCount--; // 减少当前处理的请求数
          this.next(); // 继续处理下一个图片
        });
    }
...
const loader = new ImageLoader(urls, 3); // 创建ImageLoader实例，并发限制为3
loader.load()
