...
// 控制并发数的函数
async function loadImagesWithConcurrency(urls, concurrency) {
  const results = [];
  let loaded = 0;
  const loading = [];

  function loadNext() {
    if (loaded >= urls.length) return Promise.resolve();
  ...
  while (loading.length < concurrency && loaded < urls.length) {
    loadNext();
  }

  while (loaded < urls.length) {
